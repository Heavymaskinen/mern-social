...
export default function Post(props) {
    ...
    const [values, setValues] = useState({
        ...
        likes: props.post.likes ? props.post.likes.length : -1,
        comments: props.post.comments
    })

    const clickLike = () => {
        let callApi = values.like ? postControl.unlike : postControl.like
        callApi({
            userId: jwt.user._id
        }, {
            t: jwt.token
            ...
            if (data.error) {
                ...
                setValues({...values, like: !values.like, likes: data.likes ? data.likes.length : -1})
            }
        })
    }
    ...
    return (
        <Card className={classes.card}>
            <CardHeader
                avatar={
                    <Avatar src={'/api/resources/photo/' + props.post.postedBy._id}/>
                }
