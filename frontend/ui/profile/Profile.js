...
import {userControl, auth, postControl} from '../api'
...
export default function Profile({match}) {
    ...
    const loadPosts = (user) => {
        postControl.listByUser({
            userId: user._id
        }, {
            t: jwt.token
        }).then((data) => {
            if (data.error) {
                console.log("Failed loading posts, " + data.error)
            } else {
                setPosts(data)
            }
        })
    ...
    useEffect(() => {
        ...
        userControl.read({
            ...
            if (data && data.error) {
                ...
                setValues({...values, user: data, following: follow})
                loadPosts(data)
            }
        })
    ...
    const photoUrl = userControl.getPhotoUrl(values.user._id)
